# BRIDL-REQ-003: Profiles and Inheritance
...
### BRIDL-REQ-003.1: Profile Folder Layout

1. A profile MUST be represented by a folder with a required `profile.yml` file.
2. Bridl MUST provide a JSON Schema for `profile.yml`.
3. Bridl MUST validate every loaded `profile.yml` file against the profile JSON Schema.
...
### BRIDL-REQ-003.2: Profile Identity
...
3. Bridl MUST reject profile IDs that cannot be safely referenced from the CLI.
4. Bridl SHOULD support a separate display label if human-readable names need spaces or punctuation.

### BRIDL-REQ-003.3: Profile Scope Precedence
...
### BRIDL-REQ-003.4: Profile Inheritance
...
3. Bridl MUST recursively resolve inherited profiles.
4. Bridl MUST detect inheritance cycles and report them as validation errors.
5. Bridl MUST preserve inherited profile order when building the profile stack.

### BRIDL-REQ-003.5: Implicit Default Profile
...
3. Bridl MUST avoid duplicating a profile in the resolved stack when the same profile appears explicitly and implicitly.

### BRIDL-REQ-003.6: Profile Merging

1. Bridl MUST merge resolved profile layers deterministically.

# APPLEPI-REQ-001: Project Foundation

## Overview

ApplePi is a TypeScript CLI project.
...
### APPLEPI-REQ-001.1: Runtime, Package Manager, and Language
...
### APPLEPI-REQ-001.2: Test Framework and Coverage
...
6. Tests that validate formal requirements MUST follow the traceability format required by APPLEPI-REQ-008.3.

### APPLEPI-REQ-001.3: Linting and Complexity
...
### APPLEPI-REQ-001.4: Persisted File Format Policy

1. User-editable persisted ApplePi configuration MUST use YAML instead of JSON unless the file is a JSON Schema.
2. Every user-editable YAML file format that ApplePi reads MUST have a corresponding JSON Schema.
3. ApplePi MUST validate YAML files against their JSON Schemas anywhere those files are read.
4. JSON Schema files MAY use JSON because schemas are tooling-facing validation artifacts.

### APPLEPI-REQ-001.5: Initial Dependency Set

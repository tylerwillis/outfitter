# APPLEPI-REQ-002: Settings Discovery and Validation

## Overview

ApplePi settings are the merged result of user, project, and project-local `.applepi/settings.yml` files.
The internal Settings object is the single source of resolved configuration for commands.

## Requirements

### APPLEPI-REQ-002.1: Settings Locations

1. ApplePi MUST support a user settings file at `~/.applepi/settings.yml`.
2. ApplePi MUST support a project settings file at `<project>/.applepi/settings.yml`.
3. ApplePi MUST support a project-local settings file at `<project>/.applepi/local/settings.yml`.
4. ApplePi MUST collectively refer to discovered settings files as `settings.yml` in user-facing documentation when discussing the merged settings concept.

### APPLEPI-REQ-002.2: Settings Precedence
...
4. ApplePi MUST expose the merged result as a conceptual internal `Settings` object.
...
### APPLEPI-REQ-002.3: Settings Schema

1. ApplePi MUST provide a JSON Schema for `settings.yml`.
2. ApplePi MUST validate every discovered `settings.yml` file against the settings JSON Schema before merging it.
...
### APPLEPI-REQ-002.4: Default Profile

1. The user settings file `~/.applepi/settings.yml` MUST declare a default profile after `applepi setup` completes.
2. `applepi run` MUST use the resolved default profile when no profile is selected with `-p` or `--profile`.
3. ApplePi MUST report an actionable error when no selected profile and no default profile are available.

### APPLEPI-REQ-002.5: Profile Sources in Settings
...
5. A `uri` or `github` profile source MUST be syncable by `applepi sync`.
...
10. If neither `only` nor `except` is specified, ApplePi MUST load all profiles from the source.

### APPLEPI-REQ-002.6: Remote Settings Sources
...
5. ApplePi MUST load cached remote settings files from their repository subpaths when resolving settings.
6. Local discovered settings MUST take precedence over remote settings when both define the same setting.

### APPLEPI-REQ-002.7: Cache Directory Setting
...
3. When `cache_directory` is not configured, ApplePi MUST use `~/.applepi/cache` as the default cache directory.

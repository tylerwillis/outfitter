# APPLEPI-REQ-009: Release Publishing

## Overview

ApplePi release publishing prepares package metadata from a GitHub release tag and publishes the `applepi` npm package through GitHub Actions.

## Requirements

### APPLEPI-REQ-009.1: Release Metadata Synchronization

1. The release metadata synchronization script MUST accept a release version from an explicit argument, `APPLEPI_RELEASE_VERSION`, or `GITHUB_REF_NAME`, in that precedence order.
...
5. The release metadata synchronization script MUST verify that the root package metadata it prepares belongs to the `applepi` npm package.
...
### APPLEPI-REQ-009.2: Npm Release Workflow
...
6. The npm release workflow MUST publish the public `applepi` package to the npm registry using an npm token secret.
